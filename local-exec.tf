resource "null_resource" "get-eni-list" {
  ...
  provisioner "local-exec" {
    command = "aws ec2 describe-vpc-endpoints --region=${var.aws_region} --filters Name=tag:Name,Values=test-ep --query VpcEndpoints[*].NetworkInterfaceIds.[join("," ,[NetworkInterfaceIds])] --output text >  eni_list.txt"
    environment = {
